...
def show_access_info(status):
    ...
    if status["deployed"]:
        ...
        print(
            f"   - Status: ssh -i {key_name}.pem ec2-user@{public_ip} 'docker exec -it amp-trading-system amp status'"
        )
        print(
            f"   - Monitor: ssh -i {key_name}.pem ec2-user@{public_ip} 'docker exec -it amp-trading-system amp monitor --dashboard'"
        )
        print(
            f"   - Logs: ssh -i {key_name}.pem ec2-user@{public_ip} 'docker logs amp-trading-system'"
        )
