...
class WebSocketService:
    ...
    async def subscribe_to_symbol(
        self, exchange: str, symbol: str, callback: Callable = None
    ):
    ...
    async def _maintain_connection(self, exchange: str):
        ...
        while self.running and retry_count < self.max_retries:
                ...
                async with websockets.connect(exchange_config["url"]) as websocket:
                    ...
                    async for message in websocket:
                        try:
                            ...
                            logger.error(
                                f"Message processing error for {exchange}: {e}"
                            )
            ...
            if self.running and retry_count < self.max_retries:
                logger.info(
                    f"Reconnecting to {exchange} in {self.reconnect_interval} seconds..."
                )
    ...
    def _binance_subscribe_format(self, symbol: str) -> Dict[str, Any]:
        ...
        return {"method": "SUBSCRIBE", "params": [f"{symbol.lower()}@trade"], "id": 1}
