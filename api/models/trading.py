...
class User(Base):
    ...
    __table_args__ = (Index("ix_users_status_active", "status", "is_active"),)
...
class TradingPair(Base):
    ...
    market_data = relationship(
        "MarketData", back_populates="trading_pair", cascade="all, delete-orphan"
    )
...
class Account(Base):
    ...
    account_type = Column(
        String(20), nullable=False, default="demo"
    )  # demo, live, paper
...
class MarketData(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "trading_pair_id", "timestamp", "timeframe", name="uq_market_data_unique"
        ),
    )
