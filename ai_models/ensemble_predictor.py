...
class FeatureEngineer:
    ...
    def _get_feature_names(self) -> List[str]:
        ...
        # Price features
        names.extend(
            ["returns", "high_close_ratio", "low_close_ratio", "volume_change"]
        )
...
class XGBoostModel:
    ...
    def train(self, X: np.ndarray, y: np.ndarray) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"XGBoost trained - Train: {train_score:.3f}, Val: {val_score:.3f}"
            )
...
class EnsemblePredictor:
    ...
    def train(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            xgb_results = self.xgb_model.train(
                features.technical_indicators, features.labels
            )
            lstm_results = self.lstm_model.train(
                features.price_sequences, features.labels
            )
            ...
            return {
                ...
                "meta_learner_score": self.meta_learner.score(
                    meta_features, features.labels
                ),
            }
    ...
    def _create_meta_features(self, features: FeatureSet) -> np.ndarray:
        ...
        for i in range(len(features.labels)):
            ...
            xgb_pred, xgb_proba = self.xgb_model.predict(
                features.technical_indicators[i : i + 1]
            )
            lstm_pred, lstm_proba = self.lstm_model.predict(
                features.price_sequences[i : i + 1]
            )
            cnn_pred, cnn_proba = self.cnn_model.predict(
                features.chart_patterns[i : i + 1]
            )
            ...
            meta_feature = np.concatenate(
                [
                    ...
                    xgb_proba,
                    lstm_proba,
                    cnn_proba,  # Probability distributions
                ]
            )
    ...
    def predict(self, df: pd.DataFrame) -> PredictionResult:
        ...
        try:
            ...
            lstm_pred, lstm_proba = self.lstm_model.predict(
                features.price_sequences[-1:]
            )
            cnn_pred, cnn_proba = self.cnn_model.predict(features.chart_patterns[-1:])

            # Create meta-features
            meta_feature = np.concatenate(
                [[xgb_pred, lstm_pred, cnn_pred], xgb_proba, lstm_proba, cnn_proba]
            ).reshape(1, -1)
            ...
            individual_preds = {
                "xgb": {
                    "prediction": self.class_names[xgb_pred],
                    "probabilities": xgb_proba.tolist(),
                },
                "lstm": {
                    "prediction": self.class_names[lstm_pred],
                    "probabilities": lstm_proba.tolist(),
                },
                "cnn": {
                    "prediction": self.class_names[cnn_pred],
    ...
    def save_models(self, model_dir: str):
        ...
        joblib.dump(
            self.feature_engineer.scaler, os.path.join(model_dir, "feature_scaler.pkl")
        )
        joblib.dump(
            self.feature_engineer.price_scaler,
            os.path.join(model_dir, "price_scaler.pkl"),
        )
    ...
    def load_models(self, model_dir: str):
        ...
        self.feature_engineer.scaler = joblib.load(
            os.path.join(model_dir, "feature_scaler.pkl")
        )
        self.feature_engineer.price_scaler = joblib.load(
            os.path.join(model_dir, "price_scaler.pkl")
        )
