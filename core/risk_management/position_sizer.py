...
class PositionSizer:
    ...
    def add_position(self, position_info: PositionInfo) -> bool:
        ...
        try:
            ...
            if new_total_risk > self.max_portfolio_risk:
                print(
                    f"Warning: Position rejected - would exceed portfolio risk limit ({new_total_risk:.2%} > {self.max_portfolio_risk:.2%})"
                )
                return False
    ...
    def remove_position(
        self, symbol: str, exit_price: float = None, profit_loss: float = None
    ) -> bool:
        """
