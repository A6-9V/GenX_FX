...
class EnsemblePredictor:
    ...
    async def _generate_features(
        ...
        try:
            ...
            microstructure_features = (
                await self.microstructure_engine.generate_features(data)
            )
            ...
            try:
                sentiment_features = await self.sentiment_engine.generate_features(
                    symbol
                )
    ...
    async def train_models(
        self, symbol: str, training_data: pd.DataFrame, target_column: str = "target"
    ) -> Dict[str, Any]:
    ...
    async def should_retrain(self, symbol: str) -> bool:
        ...
        retrain_interval = timedelta(
            hours=self.config.get("retrain_interval_hours", 24)
        )
