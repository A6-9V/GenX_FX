...
class ForexConnectExcelGenerator:
    ...
    def generate_signals_from_prices(self, prices, num_signals=10):
        ...
        for i in range(num_signals):
            ...
            if signal_type == "BUY":
                ...
                stop_loss = entry_price - self.calculate_atr_distance(
                    symbol, current_price
                )
                take_profit = (
                    entry_price + self.calculate_atr_distance(symbol, current_price) * 2
                )
            else:
            ...
            signal = {
                ...
                + timedelta(minutes=np.random.randint(-5, 5)),
                "Symbol": symbol_clean,
                "Signal": signal_type,
    ...
    def calculate_confidence(self, symbol, spread, price):
        ...
        final_confidence = np.random.uniform(
            base_confidence * 0.8, base_confidence * 1.0
        )
    ...
    def create_excel_dashboard(self, df, prices):
        ...
        for row_idx, (_, signal) in enumerate(df.iterrows(), 2):
            ws_active.cell(
                row=row_idx, column=1, value=signal["Timestamp"].strftime("%H:%M:%S")
            )
        ...
        for col in range(1, 13):
            ws_active.column_dimensions[openpyxl.utils.get_column_letter(col)].width = (
                12
            )
        ...
        for row_idx, (symbol, price_data) in enumerate(prices.items(), 2):
            ...
            ws_prices.cell(
                row=row_idx, column=5, value=price_data["time"].strftime("%H:%M:%S")
            )
        ...
        connection_status = (
            "🟢 LIVE (ForexConnect)" if self.connected else "🟡 DEMO (Mock Data)"
        )
